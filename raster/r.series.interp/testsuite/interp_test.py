...
class InterpolationTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        # create fixtures
        cls.runModule("r.mapcalc", expression="prec_1 = 100")
        cls.runModule("r.mapcalc", expression="prec_5 = 500")

    @classmethod
    def tearDownClass(cls):
        """Remove the temporary region """
        cls.remove_maps(rasters=["prec_1", "prec_5"])
        cls.del_temp_region()

    def tearDown(self):
        self.remove_maps(rasters=["prec_2", "prec_3", "prec_4"])
    ...
    def test_infile(self):
        self.assertModule("r.series.interp", input="prec_1,prec_5", datapos=(0.0,1.0),
            outfile=os.path.join(DATA_DIR, "outfile_1.txt"), method="linear")

        self.assertRasterMinMax(map="prec_2", refmin=200, refmax=200)
        self.assertRasterMinMax(map="prec_3", refmin=300, refmax=300)
        self.assertRasterMinMax(map="prec_4", refmin=400, refmax=400)

class InterpolationTest2(TestCase):
    ...
    def tearDown(self):
        self.remove_maps(rasters=[
            "map_10", "map_20", "map_30", "map_40",
            "map_12", "map_14", "map_16", "map_18", "map_25", "map_35",
        ])

    def test_inoutfiles(self):
        self.assertModule("r.series.interp", infile=os.path.join(DATA_DIR, "infile_2.txt"),
            outfile=os.path.join(DATA_DIR, "outfile_2.txt"), method="linear")
...
class InterpolationTestFailures(TestCase):
    @classmethod
    def setUpClass(cls):
        """Use temporary region settings"""
        cls.use_temp_region()
